...
- [#341](https://github.com/nf-core/epitopeprediction/pull/341) Enforce hard per-tool cap on alleles per chunk; `--max_alleles_per_chunk` can now only lower it, never raise it past the per-tool limit (NetMHCIIpan capped at 40 to keep the `-a` argument under the 1024-char binary limit on heterodimer alleles) ([@jonasscheid](https://github.com/jonasscheid/))
