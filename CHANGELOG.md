...
- Retained the `--proteome_reference` self/novelty filter from the old epytope path, reimplemented as a dependency-free post-processing step in `variant_fasta2peptides.py`: variant peptides occurring as a substring of any protein in the given reference proteome are dropped before prediction (only the peptide lists are filtered; the annotated protein FASTA is left complete) ([@axelwalter](https://github.com/axelwalter/))
