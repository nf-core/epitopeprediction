...
workflow GENERATE_VARIANT_PEPTIDES {
    ...
    PREP_GERMLINE_CONTEXT( ch_context.germline.map { meta, vcf -> [ meta, vcf, meta.germline_vcf, Math.max(max_length(meta) - 1, fasta_flank) ] }, ch_chr_map )
    ...
    PVACSEQ_GENERATE_FASTA(
        ch_vep_vcf.join( PREP_PROXIMAL_VCF.out.vcf ).map { meta, vcf, tbi, pvcf, ptbi -> [ meta, vcf, tbi, pvcf, ptbi, min_length(meta), max_length(meta), fasta_flank ] }
    )
