...
include {
    MHCNUGGETS ;
    MHCNUGGETS as MHCNUGGETSII
} from '../../../modules/local/mhcnuggets'
include { NETMHCPAN } from '../../../modules/local/netmhcpan'
include { NETMHCIIPAN } from '../../../modules/local/netmhciipan'
include {
    UNPACK_NETMHC_SOFTWARE as NETMHCPAN_IMPORT ;
    UNPACK_NETMHC_SOFTWARE as NETMHCIIPAN_IMPORT
} from '../../../modules/local/unpack_netmhc_software'
...
workflow MHC_BINDING_PREDICTION {
    ...
    PREPARE_PREDICTION_INPUT(ch_peptides_to_predict, supported_alleles_json).prepared.transpose().branch { meta, json, file ->
    ...
    ch_binding_predictors_out
        .map { meta, file -> [meta.findAll { k, _v -> k != 'alleles_supported' }, file] }
        .groupTuple()
...
def parse_netmhc_params(tool_name, netmhc_software_meta) {
    ...
    ch_netmhc_exe.bind(
        [tool_name, entry.version, entry.software_md5.join(' '), file(params["${tool_name}_path"], checkIfExists: true), entry.data_url ? file(entry.data_url, checkIfExists: true) : [], entry.data_md5 ? entry.data_md5 : "", entry.binary_name]
    )
