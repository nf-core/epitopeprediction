...
def group_peptides(peptides_set):
    ...
    peptides = [
        {"sequence": seq, "protein_ids": ";".join(data["protein_ids"]), "counts": data["counts"]}
...
def write_output(peptides, output_file):
    """Writes the peptides data to a TSV file without using pandas."""
    start_time = time()

    with open(output_file, "w") as f:
        # Write header
        f.write("sequence\tprotein_ids\tcounts\n")
        # Write data
        for peptide in peptides:
            f.write(f"{peptide['sequence']}\t{peptide['protein_ids']}\t{peptide['counts']}\n")
...
def main():
    ...
    for k in range(args.min_length, args.max_length + 1):
        ...
        peptides = group_peptides(peptides_set)
        output_filename = f"{args.output_prefix}_length_{k}.tsv"
        write_output(peptides, output_filename)
