...
def parse_args(args=None):
    Description = "Reformat nf-core/epitopeprediciton samplesheet file and check its contents."
...
def check_samplesheet(file_in, file_out):
    ...
    with open(file_in, "r") as fin:
        ...
        header = [x for x in fin.readline().strip().split(",")]
        ...
        for line in fin:
            ...
            ## Check if the allele column is filled according to guidelines
            if alleles.find(" ") != -1:
                print_error("Alleles entry contains spaces!", "Line", line)

            # Check if the filename column is filled according to guidelines
            if filename.find(" ") != -1:
                print_error("Filename entry contains spaces!", "Line", line)

            ## Check if the alleles given in the text file are in the right format
            if alleles.endswith(".txt"):
                with open(alleles, "r") as af:
                    alleles_from_txt = ""
                    for al in af.readlines():
                        alleles_from_txt = alleles_from_txt.strip('\n') + ';' + check_allele_nomenclature(al)
                alleles = alleles_from_txt.lstrip(';')
