...
VCFs without a `GT` field (e.g. Strelka) are accepted: the pipeline assigns the tumor sample `0/1` before
running `pvacseq`. Strelka names its columns `NORMAL` and `TUMOR`, so set `tumor_sample` to `TUMOR`, and its
SNV and indel VCFs need one samplesheet row each.
