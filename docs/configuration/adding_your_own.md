# nf-core/epitopeprediction: Configuration for other clusters
...
### Docker
Docker is a great way to run nf-core/epitopeprediction, as it manages all software installations and allows the pipeline to be run in an identical software environment across a range of systems.

Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/epitopeprediction](https://hub.docker.com/r/nfcore/epitopeprediction/) image that we have created and is hosted at dockerhub at run time.
...
process.container = "shub://nf-core/epitopeprediction"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-epitopeprediction.simg"
```
