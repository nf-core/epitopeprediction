...
if ( !params.show_supported_models ){
    ...
    ["netmhc", "netmhcpan", "netmhcii", "netmhciipan"].each {
        ...
        {
            log.warn("--${it}_path specified, but --tools does not contain ${it} have to be both specified to enable ${it}. Ignoring.")
        }
        else if (!params["${it}_path"] as Boolean && tools.contains(it))
        {
            log.warn("--${it}_path not specified, but --tools contains ${it} have to be both specified to enable ${it}. Ignoring.")
...
process netmhc_tools_import {
    ...
    #
    # MODIFY THE NETMHC WRAPPER SCRIPT SO THAT IT FINDS ITS OWN LOCATION
    #
    sed -i.bak -e "s_/scratch_/tmp_" -e "s_setenv[[:space:]]NMHOME.*_setenv NMHOME \\`realpath -s \\\$0 | sed -r 's/[^/]+\$//'\\`_" "${toolname}/${toolbinaryname}"
